...
class DestinationResolver:
    ...
    # ---------------------- startup -----------------------
    def __init__(self, yaml_path: str, fuzzy_cut=None, embed_cut=None,
                 embedding_model=None, device="cpu"):
        with open(yaml_path) as f:
            full_cfg = yaml.safe_load(f)
        dest_raw = full_cfg["destinations"]
        self.raw = dest_raw
        models_cfg = full_cfg.get("models", {}) or {}
        dr_cfg = full_cfg.get("destination_resolver", {}) or {}
        if fuzzy_cut is None:
            fuzzy_cut = dr_cfg.get("fuzzy_cutoff", 80)
        if embed_cut is None:
            embed_cut = dr_cfg.get("embedding_cutoff", 0.55)
        if embedding_model is None:
            embedding_model = models_cfg.get("embedding_model", "paraphrase-MiniLM-L6-v2")
