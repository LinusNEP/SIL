...
class ObjectTracker:
    def __init__(self, initial_pose, process_noise=None,
                 measurement_noise=None, initial_velocity_cov=None):
        if process_noise is None:
            process_noise = rospy.get_param("perception/kalman_process_noise", 1e-4)
        if measurement_noise is None:
            measurement_noise = rospy.get_param("perception/kalman_measurement_noise", 0.1)
        if initial_velocity_cov is None:
            initial_velocity_cov = rospy.get_param(
                "perception/kalman_initial_velocity_covariance", 1000.0)
        self.kf = KalmanFilter(dim_x=6, dim_z=3)
        self.kf.F = np.eye(6)
        self.kf.F[:3, 3:] = np.eye(3)
        self.kf.H = np.eye(3, 6)
        self.kf.R = np.eye(3) * measurement_noise
        self.kf.P[3:, 3:] *= initial_velocity_cov
...
class PerceptionModule:
    def __init__(self, data_logger=None):
        ...
        clip_model_name = rospy.get_param("models/clip_model", "openai/clip-vit-base-patch32")
        self.clip_model = CLIPModel.from_pretrained(clip_model_name).to(self.device)
        self.clip_processor = CLIPProcessor.from_pretrained(clip_model_name)
        sam_checkpoint = rospy.get_param("models/sam_checkpoint", "")
        sam_model_type = rospy.get_param("models/sam_model_type", "vit_b")
        ...
        if not os.path.exists(sam_checkpoint):
            rospy.logerr(f"SAM checkpoint not found at {sam_checkpoint}. Please check the path.")
            raise FileNotFoundError(f"SAM checkpoint not found at {sam_checkpoint}.")
        self.sam_model = sam_model_registry[sam_model_type](checkpoint=sam_checkpoint).to(self.device)
        self.sam_predictor = SamPredictor(self.sam_model)
        self.mask_generator = SamAutomaticMaskGenerator(self.sam_model)
        midas_model = rospy.get_param("models/midas_model", "MiDaS_small")
        midas_transform = rospy.get_param("models/midas_transform", "small_transform")
        self.mono_model = torch.hub.load('intel-isl/MiDaS', midas_model).to(self.device)
        self.mono_transform = getattr(
            torch.hub.load('intel-isl/MiDaS', 'transforms'), midas_transform)
        ...
        self.image_publisher = rospy.Publisher(rospy.get_param("topics/llm_image_output", "/llm_image_output"), Image, queue_size=10)
        ...
        self.vocabulary_prompts = rospy.get_param(
            "perception/vocabulary_prompts", self.object_labels)
        self.use_command_vocabulary = bool(
        ...
        if self.perception_backend != "clip_sam":
            try:
                from sil_ros.perception_backends import create_backend
                self._open_vocab_backend = create_backend(
                    self.perception_backend,
                    get_param=lambda k, d=None: rospy.get_param(k, d),
                    device=str(self.device),
                )
            except Exception as e:
                rospy.logwarn(f"[Perception] Could not init backend "
                              f"'{self.perception_backend}': {e}. Using clip_sam.")
                self._open_vocab_backend = None
        if self._open_vocab_backend is not None:
            rospy.loginfo(f"[Perception] Open-vocabulary backend active: "
                          f"{self.perception_backend}")
    ...
    def setup_subscribers(self):
        ...
        ats = message_filters.ApproximateTimeSynchronizer([rgb_sub, depth_sub], queue_size=10, slop=rospy.get_param("perception/image_sync_slop", 0.1))
    ...
    def _resolve_queries(self, queries=None):
        if queries and self.use_command_vocabulary:
            return [q for q in queries if q]
        return list(self.vocabulary_prompts)

    def detect_objects(self, prob_thresh=None, color_thresh=None, queries=None):
    ...
    def _detect_open_vocab(self, queries, prob_thresh):
        rospy.loginfo(f"[Perception] Open-vocab detection for concepts: {queries}")
        rgb = self.image_manager.rgb_image
        height, width = rgb.shape[:2]
        try:
            regions = self._open_vocab_backend.detect(rgb, queries)
        except Exception as e:
        ...
        for region in regions:
            if region["score"] < prob_thresh:
                continue
            mask = region.as_mask(height, width)
            ...
            if rospy.get_param("debug/save_debug_masks", False):
                self.save_debug_mask(mask, region["label"])
        rospy.loginfo(f"[Perception] Open-vocab accepted {len(detections)} "
                      f"detections from {len(regions)} candidates.")
        return detections
    ...
    def get_object_locations(self, queries=None):
        ...
        for det in (detections or []):
            ...
            pos = det.get("position") or det.get("pose")
            if pos is None:
                continue
            entry = {
                "x": pos.x,
        ...
        try:
            ...
            try:
                ...
                for mask, score in zip(masks, scores):
                    if score < self.sam_mask_score_threshold:
                        continue
            ...
            if rospy.get_param("debug/save_debug_masks", False) and valid_masks:
    ...
    def get_mask_depth(self, mask, centroid, fallback_to_mono=True):
        try:
            ...
            depth_val = self.depth_estimator.get_valid_depth_value(depth_image, x, y, radius=self.depth_valid_radius)
